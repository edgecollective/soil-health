...
void setup(){
  ...
  for(byte i = '0'; i <= '9'; i++) if(checkActive(i)) {
    numSensors++; setTaken(i);
    
    }   // scan address space 0-9

  for(byte i = 'a'; i <= 'z'; i++) if(checkActive(i)) {
    numSensors++; setTaken(i);
  ...
  for(byte i = 0; i < 62; i++){
    if(isTaken(i)){
      ...
      Serial.print("Setting sensor_address to ");
      Serial.println(i);
      sensor_address=i;
      break;
    }
